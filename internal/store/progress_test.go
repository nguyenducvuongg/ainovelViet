...
func TestSetNovelName(t *testing.T) {
	...
	if err := store.Progress.SetNovelName("Đèn đốt đêm dài"); err != nil {
		t.Fatalf("SetNovelName: %v", err)
	}

	p, _ := store.Progress.Load()
	if p.NovelName != "Đèn đốt đêm dài" {
...
func TestSetPendingRewrites(t *testing.T) {
	...
	chapters := []int{3, 5, 7}
	if err := store.Progress.SetPendingRewrites(chapters, "Động cơ của nhân vật không nhất quán"); err != nil {
	...
	if p.RewriteReason != "Động cơ của nhân vật không nhất quán" {
...
func TestSetPendingRewritesRejectsUnfinishedChapters(t *testing.T) {
	...
	if err := store.Progress.SetPendingRewrites([]int{3, 5}, "Bài kiểm tra"); err == nil {
...
func TestCompleteRewrite(t *testing.T) {
	...
	_ = store.Progress.MarkChapterComplete(5, 3000, "", "")
	_ = store.Progress.MarkChapterComplete(7, 3000, "", "")
	_ = store.Progress.SetPendingRewrites([]int{3, 5, 7}, "viết lại bài kiểm tra")
	...
	// Hoàn thành chương cuối → Tự động thiết lập lại Flow
...
func TestCompleteRewrite_NotInQueue(t *testing.T) {
	...
	_ = store.Progress.MarkChapterComplete(3, 3000, "", "")
	_ = store.Progress.MarkChapterComplete(5, 3000, "", "")
	_ = store.Progress.SetPendingRewrites([]int{3, 5}, "Bài kiểm tra")

	// Việc hoàn thành các chương không có trong hàng đợi sẽ không gây ra lỗi
...
func TestClearPendingRewrites(t *testing.T) {
	...
	_ = store.Progress.MarkChapterComplete(3, 3000, "", "")
	_ = store.Progress.SetPendingRewrites([]int{1, 2, 3}, "Bài kiểm tra")
	_ = store.Progress.SetFlow(domain.FlowRewriting)
