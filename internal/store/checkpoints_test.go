...
func TestCheckpointStore_EmptyDigestNotIdempotent(t *testing.T) {
	...
	// Thông báo trống không tham gia vào quá trình loại bỏ trùng lặp idempotent
...
func TestCheckpointStore_Reset(t *testing.T) {
	...
	// đặt lại seq sau khi đặt lại: phần bổ sung tiếp theo bắt đầu từ 1
...
func TestCheckpointStore_RestoreFromDisk(t *testing.T) {
	...
	// Khởi động lại mô phỏng: phiên bản mới được tải từ cùng thư mục
	io2 := newIO(dir)
	...
	// seq nên tiếp tục từ 4 và tính bình thường vẫn được áp dụng
...
func TestCheckpointStore_SeqNotConsumedOnWriteFailure(t *testing.T) {
	...
	// Thay đổi chính tệp jsonl thành chỉ đọc để lần ghi OpenFile tiếp theo không thành công
	...
	// bộ nhớ đệm không nên bị ô nhiễm
	if all := cs.All(); len(all) != 1 {
		t.Fatalf("cache leaked failed entry, len=%d", len(all))
	}

	// Khôi phục quyền ghi, thử lại sẽ cho kết quả seq=2 thay vì seq=3
