...
func (s *askUserState) choiceDescription(idx int) string {
	...
	return "Không có điều nào ở trên phù hợp, hãy thêm của riêng bạn"
}
...
func renderAskUserModal(width, height int, state *askUserState) string {
	...
	title := fmt.Sprintf("Cần thêm thông tin %d/%d", state.index+1, len(state.request.questions))
	...
	hint := "↑↓ Chọn · Nhập để xác nhận · Esc để đóng"
	if q.MultiSelect {
		hint = "↑↓ Chọn · Dấu cách để kiểm tra · Enter để gửi · Esc để đóng"
	}
	if state.typing {
		hint = "Nhập nội dung bổ sung · Enter để xác nhận · Esc để quay lại lựa chọn"
	}
	b.WriteString("\n")
