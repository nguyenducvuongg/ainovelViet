...
func (h *terminalAskUser) askOne(ctx context.Context, q tools.Question) (string, string, error) {
	...
	fmt.Fprintln(h.out, "  0. Đầu vào tùy chỉnh")
	...
	for {
		...
		if q.MultiSelect {
			fmt.Fprint(h.out, "Hãy nhập số, ngăn cách nhiều số bằng dấu phẩy: ")
		} else {
			fmt.Fprint(h.out, "Vui lòng nhập số: ")
		}
		...
		if line == "" {
			fmt.Fprintln(h.out, "Đầu vào không được để trống, vui lòng thử lại.")
			continue
		}
		if line == "0" {
			fmt.Fprint(h.out, "Vui lòng nhập nội dung tùy chỉnh: ")
			note, err := h.readLine()
			if err != nil {
				return "", "", err
			}
...
func parseSelections(line string, options []tools.Option, multi bool) ([]string, error) {
	...
	if !multi && len(parts) > 1 {
		return nil, fmt.Errorf("Câu hỏi hiện tại chỉ cho phép lựa chọn duy nhất")
	}
	...
	for _, part := range parts {
		...
		if part == "" {
			return nil, fmt.Errorf("Số không thể trống")
		}

		var idx int
		if _, err := fmt.Sscanf(part, "%d", &idx); err != nil {
			return nil, fmt.Errorf("Số không được nhận dạng %q", part)
		}
		if idx <= 0 || idx > len(options) {
			return nil, fmt.Errorf("Số %d nằm ngoài phạm vi", idx)
		}
		if seen[idx] {
			continue
		}
	...
	if len(labels) == 0 {
		return nil, fmt.Errorf("Chọn ít nhất một tùy chọn")
	}
	return labels, nil
}
