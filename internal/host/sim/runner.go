...
func Run(ctx context.Context, deps Deps, opts Options) (<-chan Event, error) {
	...
	go func() {
		...
		emit(StageScan, 0, 0, "Quét mô phỏng kho văn bản...", nil)
		sources, err := scanSources(opts.SourceDir)
		if err != nil {
			emit(StageError, 0, 0, "Quét thư mục mô phỏng không thành công", err)
			return
		}
		if len(sources) == 0 {
			emit(StageError, 0, 0, "Không có tệp .txt/.md/.markdown có thể phân tích cú pháp trong thư mục mô phỏng", fmt.Errorf("no simulation sources"))
			return
		}

		existing, err := deps.Store.Simulation.Load()
		if err != nil {
			emit(StageError, 0, len(sources), "Không thể đọc được hình ảnh hiện có", err)
			return
		}
		pending := pendingSources(existing, sources)
		if len(pending) == 0 {
			emit(StageDone, 0, len(sources), "Hình ảnh được cập nhật, không tìm thấy bài viết mới hoặc thay đổi", nil)
			return
		}
		...
		for i, source := range pending {
			if err := ctx.Err(); err != nil {
				emit(StageError, i, len(pending), "Phân tích hồ sơ hủy của người dùng", err)
				return
			}
			emit(StageAnalyze, i+1, len(pending), fmt.Sprintf("Phân tích kho ngữ liệu giả %d/%d: %s", i+1, len(pending), source.RelativePath), nil)
			report, err := AnalyzeSource(ctx, deps.LLM, deps.Prompts.Source, source)
			if err != nil {
				emit(StageError, i+1, len(pending), "Phân tích Corpus không thành công", err)
				return
			}
		...
		emit(StageMerge, len(pending), len(pending), "Hợp nhất các bức chân dung mô phỏng...", nil)
		synthesis, err := MergeSynthesis(ctx, deps.LLM, deps.Prompts.Merge, existing, allReports)
		if err != nil {
			emit(StageError, len(pending), len(pending), "Hợp nhất hình ảnh không thành công", err)
			return
		}
		...
		if err := deps.Store.Simulation.Save(profile); err != nil {
			emit(StageError, len(pending), len(pending), "Không lưu được hình ảnh mô phỏng", err)
			return
		}
		emit(StageDone, len(pending), len(pending), fmt.Sprintf("Chân dung giả đã được cập nhật: Đã thêm/thay đổi %d bài viết, thêm tổng số %d bài viết", len(pending), len(profile.Corpus.Sources)), nil)
	}()
	return events, nil
}
