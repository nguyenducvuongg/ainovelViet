...
func buildWriterStoreSummaryText(s *store.Store, budgetTokens int) (string, bool, error) {
	...
	return "Sau đây là từ kho lưu trữ lưu trữ mới, được sử dụng để khôi phục bối cảnh viết sau khi nén. \n\n" + strings.Join(parts, "\n\n"), true, nil
}
...
func writerStoreSummarySections(state *writerStoreSummaryState) []writerStoreSection {
	return []writerStoreSection{
		...
		{heading: "Tóm tắt các chương gần đây", data: state.recentSummaries},
		{heading: "Kế hoạch chương hiện tại", data: state.chapterPlan},
		{heading: "Đề cương chương hiện tại", data: state.currentOutline},
		{heading: "Tóm tắt hồ quang hiện tại", data: state.currentArcSummary},
		{heading: "Tóm tắt khối lượng hiện tại", data: state.currentVolSummary},
		{heading: "ảnh chụp nhanh nhân vật", data: state.snapshots},
		{heading: "Điềm báo chủ động", data: state.foreshadow},
		{heading: "Xem xét các vấn đề cần sửa đổi", data: state.pendingReviews},
		{heading: "dòng thời gian gần đây", data: state.timeline},
		{heading: "quy tắc phong cách", data: state.styleRules},
	}
}
