...
// Công cụ IO thuần túy: chỉ chịu trách nhiệm tải dữ liệu và không đưa ra hướng dẫn.
...
func (t *CheckConsistencyTool) Description() string {
	return "Tải các bản nháp bằng văn bản và dữ liệu so sánh (các quy tắc chung, điềm báo, mối quan hệ, bí danh, tóm tắt gần đây) để bạn kiểm tra tính nhất quán. Phải được gọi sau Draft_chapter"
}
func (t *CheckConsistencyTool) Label() string { return "Kiểm tra tính nhất quán" }

// Công cụ chỉ đọc (chỉ bổ sung các sự kiện điểm kiểm tra, không thay đổi trạng thái) và có thể được lên lịch đồng thời.
...
func (t *CheckConsistencyTool) Schema() map[string]any {
	return schema.Object(
		schema.Property("chapter", schema.Int("Số chương để kiểm tra")).Required(),
	)
}
...
func (t *CheckConsistencyTool) Execute(_ context.Context, args json.RawMessage) (json.RawMessage, error) {
	...
	// Kiểm soát dữ liệu: giữ lại dữ liệu kiểm tra tính nhất quán toàn cầu để tránh tải lặp lại dữ liệu cửa sổ hiện có của Novel_context
