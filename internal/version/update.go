...
func Update(ctx context.Context, opts UpdateOptions) (*UpdateResult, error) {
	if runtime.GOOS == "windows" {
		return nil, fmt.Errorf("Windows không hỗ trợ tự cập nhật tại chỗ. Vui lòng truy cập https://github.com/%s/releases để tải xuống phiên bản mới.", opts.Repo)
	}
	if opts.Repo == "" {
	...
	if rel.TagName == "" {
		return nil, fmt.Errorf("bản phát hành bị thiếu tag_name")
	}
...
func selectAsset(rel *release, binaryName string) (releaseAsset, error) {
	...
	return releaseAsset{}, fmt.Errorf("phát hành %s Không tìm thấy gói cài đặt nền tảng hiện tại *%s", rel.TagName, suffix)
}
...
func assetSuffix() (string, error) {
	...
	case "linux":
		osName = "Linux"
	default:
		return "", fmt.Errorf("Hệ thống không được hỗ trợ %s", runtime.GOOS)
	}
	var arch string
